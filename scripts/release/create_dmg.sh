...
usage() {
  cat <<'EOF'
Usage: create_dmg.sh <app-path> <output-dmg> <volume-name> <icon-png-path>
EOF
}
...
swift "${script_dir}/render_dmg_background.swift" "${background_path}" "${volume_name}" "${icon_path}"
...
if ! run_with_timeout 45 osascript "${script_dir}/apply_dmg_layout.applescript" "${volume_name}" "${app_name}"; then
  echo "Warning: failed to apply Finder layout; continuing with basic DMG contents." >&2
fi
