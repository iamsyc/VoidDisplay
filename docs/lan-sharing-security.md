...
主屏别名始终解析到当前系统主屏。当主屏切换时，别名跟随新的主屏；具体显示器地址仍指向原 `shareID`。当前主屏没有处于分享注册集时，主屏别名不可用。UI 只展示具体显示器页面地址。

停止分享会立即撤销该凭证。再次开始分享会生成新凭证，旧链接无法复用。无凭证旧路由和错误凭证统一返回 `404`，路由不会借此暴露目标是否存在或是否正在分享。
