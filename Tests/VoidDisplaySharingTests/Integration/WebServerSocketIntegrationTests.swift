...
struct WebServerSocketIntegrationTests {
    ...
    @Test func displayRouteEmbedsBootstrapJSON() async throws {
        ...
        #expect(responseText.contains("function registerDisplayPageCodec"))
        #expect(responseText.contains("function registerDisplayPageStats"))
        #expect(responseText.contains("function registerDisplayPagePeer"))
        #expect(responseText.contains("function registerDisplayPageConnection"))
        #expect(responseText.contains("function startDisplayPageRuntime"))
        let uiRange = try #require(responseText.range(of: "function registerDisplayPageUI"))
        let codecRange = try #require(responseText.range(of: "function registerDisplayPageCodec"))
        let statsRange = try #require(responseText.range(of: "function registerDisplayPageStats"))
        let peerRange = try #require(responseText.range(of: "function registerDisplayPagePeer"))
        let connectionRange = try #require(responseText.range(of: "function registerDisplayPageConnection"))
        let runtimeRange = try #require(responseText.range(of: "function startDisplayPageRuntime"))
        #expect(uiRange.lowerBound < codecRange.lowerBound)
        #expect(codecRange.lowerBound < statsRange.lowerBound)
        #expect(statsRange.lowerBound < peerRange.lowerBound)
        #expect(peerRange.lowerBound < connectionRange.lowerBound)
        #expect(connectionRange.lowerBound < runtimeRange.lowerBound)
