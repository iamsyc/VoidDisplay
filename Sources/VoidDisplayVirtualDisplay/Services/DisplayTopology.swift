...
import CoreGraphics
import OSLog
package struct DisplayTopologySnapshot: Equatable {
    package struct DisplayInfo: Equatable {
    ...
    package let mainDisplayID: CGDirectDisplayID
    package let displays: [DisplayInfo]

    package func display(for id: CGDirectDisplayID) -> DisplayInfo? {
        displays.first(where: { $0.id == id })
    }
}

package extension DisplayTopologySnapshot.DisplayInfo {
    var isViable: Bool {
        isActive && bounds.width > 1 && bounds.height > 1
    }
}
package protocol DisplayTopologyInspecting {
    func snapshot(
...
package protocol DisplayTopologyRepairing {
    func repair(
...
package func makeSystemManagedDisplayOnlineChecker(
    managedVendorID: UInt32,
    managedProductID: UInt32
) -> (UInt32) -> Bool {
    { serialNum in
...
package struct SystemDisplayTopologyInspector: DisplayTopologyInspecting {
...
package struct SystemDisplayTopologyRepairer: DisplayTopologyRepairing {
