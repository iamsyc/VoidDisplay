...
package final class MainDisplayPolicyResolver {
    ...
    package struct PolicyResolution {
        let applies: Bool
    ...
    package func resolveMainDisplayPolicy(
    ...
    package func reconcileMainDisplayPolicyIfNeeded(
        snapshot: DisplayTopologySnapshot?,
    ...
    package func markAggressiveRecoveryPending(configId: UUID) {
        aggressiveRecoveryPendingEnableConfigIDs.insert(configId)
    }

    package func isAggressiveRecoveryPending(configId: UUID) -> Bool {
        aggressiveRecoveryPendingEnableConfigIDs.contains(configId)
    }

    package func markAggressiveRecoveryPendingForSerial(_ serialNum: UInt32) {
    ...
    package func clearAggressiveRecoveryPending(configId: UUID) {
        aggressiveRecoveryPendingEnableConfigIDs.remove(configId)
    }

    package func clearAggressiveRecoveryPendingForSerial(_ serialNum: UInt32) {
        let configs = allConfigsProvider()
        let ids = configs
