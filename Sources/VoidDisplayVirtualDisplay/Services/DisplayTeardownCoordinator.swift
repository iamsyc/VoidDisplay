...
@MainActor
package struct TeardownSettlement {
    package let terminationObserved: Bool
    package let offlineConfirmed: Bool
}

@MainActor
package protocol DisplayTeardownCoordinating: AnyObject {
...
@MainActor
package final class DisplayTeardownCoordinator: DisplayTeardownCoordinating {
    ...
    package func setRuntimeGenerationProvider(_ provider: @escaping (UUID) -> UInt64?) {
        runtimeGenerationProvider = provider
    }

    package func setReconfigurationMonitorAvailable(_ isAvailable: Bool) {
    ...
    package func waitForManagedDisplaysOffline(
    ...
    package func waitForTeardownSettlement(
        configId: UUID,
        expectedGeneration: UInt64,
        serialNum: UInt32,
        terminationTimeout: TimeInterval,
    ...
    package func settleRebuildTeardown(
        configId: UUID,
        serialNum: UInt32,
        generationToWaitFor: UInt64?,
        rebuildTerminationTimeout: TimeInterval,
    ...
    package func observeTermination(configId: UUID, generation: UInt64) {
        AppLog.virtualDisplay.debug(
    ...
    package func completeOfflineWaitersIfPossible() {
    ...
    package func cancelAllOfflineWaiters() {
        let tokens = offlineWaitersByToken.keys
        for token in tokens {
