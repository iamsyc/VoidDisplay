...
@MainActor
package final class ScreenCaptureDisplayCatalogLoader {
    package typealias LoadShareableDisplays = @MainActor () async throws -> [SCDisplay]
    package typealias OnDisplaysLoaded = @MainActor ([SCDisplay]) -> Void
    ...
    package let state: ScreenCaptureDisplayCatalogState
    ...
    package func loadDisplaysIfNeeded(onLoaded: @escaping OnDisplaysLoaded = { _ in }) {
        guard !state.isLoadingDisplays, state.displays == nil else { return }
        loadDisplays(onLoaded: onLoaded)
    }

    package func loadDisplays(
        ...
        let task = Task { @MainActor [weak self] in
            ...
            do {
                ...
                guard self.canCommitDisplayLoadResult(requestID: requestID) else { return }
                self.logger.error(
                    "\(self.logOperation, privacy: .public) failed: \(String(describing: error), privacy: .public)"
                )
    ...
    package func cancelInFlightDisplayLoad() {
        displayLoadTask?.cancel()
        displayLoadTask = nil
    ...
    package func clearDisplaysAndCancel() {
        cancelInFlightDisplayLoad()
        state.displays = nil
    }
